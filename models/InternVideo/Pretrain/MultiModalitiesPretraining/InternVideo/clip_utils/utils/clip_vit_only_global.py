...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x, cluster_subject, remove_feat=False):
        self.attn_mask = self.attn_mask.to(dtype=x.dtype, device=x.device) if self.attn_mask is not None else None
        if not cluster_subject is None:
            return self.attn(x, x, x, need_weights=False, attn_mask=self.attn_mask, cluster_subject=cluster_subject, remove_feat=remove_feat)
        else:
            return self.attn(x, x, x, need_weights=False, attn_mask=self.attn_mask, cluster_subject=cluster_subject, remove_feat=remove_feat)[0]

    def forward(self, x, T=8, use_checkpoint=False, cluster_subject=None, remove_feat=False):
        # x: 1+HW, NT, C
        # MHSA
        clust_feat = None
        if use_checkpoint:
            ...
            x_attn, clust_feat = self.attention(self.ln_1(x), cluster_subject=cluster_subject)
    ...
    def forward(self, x, mode='video', return_all_feats=False):
        ...
        for i, resblock in enumerate(self.resblocks):
            if self.use_checkpoint and i < self.checkpoint_num[0]:
                ...
                x, features  = resblock(x, T_down, cluster_subject=self.cluster_subject)

            # extract features
            if self.cluster_layer is not None and i in self.cluster_layer:
                clust_features.append(features)
        ...
        if self.cluster_subject == 'attn':
            if self.attn_head == [-1]:
                clust_features = [feat.mean(1) for feat in clust_features]
            else:
                clust_features = [feat[:, self.attn_head] for feat in clust_features]
        else:
            if self.attn_head == [-1]:
                clust_features = [feat.mean(2) for feat in clust_features]
            else:
                clust_features = [feat[:, :, self.attn_head, :] for feat in clust_features]
            clust_features = [rearrange(feat, 'B (H W T) nh C -> B C nh T H W', B=1, T=16, H=14, W=14).cpu() for feat in clust_features]



        return feats, clust_features
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, mode='video', return_all_feats=False):
        ...
        out, clust_features = self.transformer(x, mode=mode, return_all_feats=return_all_feats)
        return out, clust_features
