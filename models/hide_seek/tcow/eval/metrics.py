...
def calculate_metrics_mask_track(data_retval, model_retval, source_name=None):
    '''
    ...
    out_mask = model_retval['output_mask']
    target_mask = model_retval['target_mask']
    output_mask_binary = (out_mask > 0.0).bool()  # (B, Q?, 1/3, T, Hf, Wf).
    ...
    # NOTE: These operations will automatically broadcast the smaller to the larger array, for
    # example AOT will "predict" snitch/occl/cont all with exactly the same mask!
        ...
        for q in range(Q):
            for t in range(T):
                ...
                if Cmt >= 2 and target_mask_areas[b, q, 1, t] > 0:
                    ...
                    # Special case!
                    # How well does the predicted snitch mask cover the GT frontmost?
                    # NOTE: this calculation is wrong, but we don't care anymore anyway.
                    # cross_sfo_ious.append(
                    #     intersection_areas[b, q, 0, t] / (union_areas[b, q, 1, t] + 1e-7))
