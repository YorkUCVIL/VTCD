...
# Internal imports.
from models.hide_seek.tcow.TimeSformer.timesformer.models.vit import TimeSformer
...
class DenseTimeSformer(torch.nn.Module):
    ...
    def __init__(self, logger, pretrained, pretrained_path, frame_height, frame_width,
                 ...
                 norm_embeddings, drop_path_rate, network_depth, cluster_subject='tokens', cluster_layer=None):
        super().__init__()
        self.logger = logger
        ...
        self.cluster_subject = cluster_subject
    ...
    def forward(self, input_pixels, extra_token_in):
        ...
        y = x
        # DEBUG:
        # for blk in self.timesformer.model.blocks[2:3]:

        concept_cluster = []
        for block_num, blk in enumerate(self.timesformer.model.blocks):
            cluster_subject = self.cluster_subject if block_num in self.cluster_layer else None
            y, cluster_feature = blk(y, B, T, W, cluster_subject, self.use_temporal_attn, self.attn_head)

            # concept discovery
            if self.cluster_layer is not None and block_num in self.cluster_layer:
                if 'attn' in cluster_subject:
                    if self.use_temporal_attn:
                        clust_out = rearrange(cluster_feature, 'B (H W T) Nh C-> B C Nh T H W', T=T, H=self.Ho, W=self.Wo).cpu()
                    else:
                        clust_out = rearrange(cluster_feature, '(B T) Nh (H W) C-> B C Nh T H W', T=T, H=self.Ho, W=self.Wo).cpu()
                else:
                    clust_out = rearrange(cluster_feature, 'B (H W T) Nh C-> B C Nh T H W', T=T, H=self.Ho, W=self.Wo).cpu()
...
x.shape  # (1, 9001, 768) = (B, T*h*w, D)
y.shape  # (1, 9001, 768) = (B, T*h*w, D)
xo = rearrange(y[:, 1:], 'B (H W T) D -> B D T H W', B=B, T=T, H=self.Ho, W=self.Wo, D=self.output_feature_dim)
xo.shape  # (1, 768, 30, 15, 20) = (B, D, T, h, w)
del x.grad, y.grad, xo.grad
x.retain_grad(), y.retain_grad(), xo.retain_grad()
xo[:, :, 10, :, :].sum().backward(retain_graph=True)
print(xo.grad.sum(dim=(0, 1, 3, 4)))
xigrad = rearrange(x.grad[:, 1:], 'B (H W T) D -> B D T H W', B=B, T=T, H=self.Ho, W=self.Wo, D=self.output_feature_dim)
print(xigrad.sum(dim=(0, 1, 3, 4)) * 1e9)
        '''
        ...
        return (output_features, extra_token_out, concept_cluster)
...
class MyDenseTimeSformerBackbone(DenseTimeSformer):
    ...
    def __init__(self, logger, num_frames=16, frame_height=224, frame_width=288,
                 ...
                 drop_path_rate=0.1, network_depth=12,cluster_subject='token'):
        super().__init__(
            logger, pretrained, pretrained_path, frame_height, frame_width, patch_dim, in_channels, num_frames,
            attention_type, causal_attention, norm_embeddings, drop_path_rate, network_depth,cluster_subject)
