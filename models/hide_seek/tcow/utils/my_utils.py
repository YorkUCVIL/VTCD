'''
Miscellaneous tools / utils / helper methods.
...
def get_checkpoint_epoch(checkpoint_path):
    ...
    if os.path.exists(epoch_path):
        ...
        try:
            ...
            print('WARNING: Could not load epoch from checkpoint path: {}'.format(checkpoint_path))
            epoch = 0
...
def sample_query_inds(B, Qs, inst_count, target_desirability, train_args, device, phase):
    ...
    for b in range(B):
        ...
        # if 'mask_track' in train_args.which_seeker:
        Qt = inst_count[b].item()  # = K = Number of VALO instances for this example.

        try:
            if train_args.query_bias == 'none':
                to_rank = torch.rand(Qt, dtype=torch.float64, device=device)
            # elif train_args.query_bias == 'complexity':
            #     to_rank = target_complexity[b, :Qt, 0]  # NOTE: Does not exist if mask_track.
            elif train_args.query_bias == 'desirability':
