...
bash gen_dset/loop_cv_kubcon_v10.sh

Aligned with: kubbench_v3.

Changes compared to v9:
- Added full divided xray renderings for all objects (rgb, depth, segm).
- Disabled example query point tracking for now.
- Disabled multiple views or perturbations for now.
- Decreased frame rate from 16 to 12 to match MOVi.
- Decreased frame count from 44 to 36.

Notes / fixes before v11:
- Unfortunately, VALO stuff has caused index mismatch in segmentation vs instances list in some
    scenes (I think?). TODX dispose of this and just use all foreground instances!
'''
