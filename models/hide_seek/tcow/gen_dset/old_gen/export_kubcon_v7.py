...
On CV:
bash gen_dset/loop_cv_kubcon_v7.sh
...
- Explicitly perturb object positions for every variation.
- Increased camera max movement_speed from 5 to 8, changed inner_radius from 9 to 10.
- Changed velocity, scale ranges, spawn regions, friction, and restitution parameters.

Notes / fixes before v8:
- Too much random falling, not enough collisions or interaction => fixed & restarted for v7.
- Also make object counts vary more
- IMPORTANT: Camera yaw was accidentally still fixed (due to multi-view) => randomized for v8.
- IMPORTANT: Slightly increased restitution (for bounciness) for v8.
- IMPORTANT: Hats as containers never existed due to bug => fixed for v8.
- IMPORTANT: Forgot to adjust dynamic object mass with scaling factor => fixed for v8.
'''
...
if 'TRI' in platform.node():
    ...
    num_views = 3
    num_queries = 128
    perturbs_first_scenes = 200
    views_first_scenes = 400
...
def do_scene(scene_idx, logger, scene_dp, scene_dn):
    ...
    for perturb_idx in range(used_num_perturbs):
        ...
        for view_idx in range(used_num_views):
            ...
            t = my_kubric.set_camera_yaw(view_idx * 360.0 / used_num_views)
