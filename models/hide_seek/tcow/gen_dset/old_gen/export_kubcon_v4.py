...
python gen_dset/export_kubcon_v4.py

rm -rf /tmp/*

pkill -9 python

Changes compated to v3:
- Finally made multiprocessing work by running imports locally and carefully.
- Camera position is mostly fixed such that (x, y) grid is less ambiguous.
- Fixed linear camera motion crucial random direction bug.
- Grid spacing is 1.5m instead of 1m.

Notes during debugging:
- Frame rate 14 seems to be a visual sweet spot, but it must divide 240, so not possible.

Conclusions / changes during v4 + before v5:
- Memory leak fixed by isolating processes.
- Number of CPU cores / threads for rendering overridden depending on number of workers.
- Some snitches are spawned too low; so increased at_z, vel_z from 4, -3 to 4.5, -3.5.
- The /tmp/ directory still gets used regardless of scratch_dir, so we must clean it periodically.
'''
