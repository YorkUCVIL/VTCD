...
bash gen_dset/loop_cv_kubcon_v9.sh

Aligned with: N/A.

Changes compared to v8:
- Made mass / density more realistic by using mass_est_fp + randomization.
- Updated mass scaling law from ^1 to ^3.
- Now randomizing friction and restitution with range 0.1.
- Fixed wrong shoe in container list.
- Fixed wrong lighting angles => no more dome random rotations.
- Adjusted focal length from 36 to 34 (=> slightly wider field of view).

Notes / fixes before v10:
- Consider splitting backgrounds & objects between train & test (like MOVi does).
  => Actually TAP-Vid/Net does NOT do this! So we will probably not do it either.
- Consider adding random kubasic objects (such that evaluation scenes with them remain in-distribution).
- Add full segmentation masks during occlusions, and label container / containee / occluder / occludee.
'''
...
num_views = 3
num_queries = 192
perturbs_first_scenes = 100  # Only test.
views_first_scenes = 100  # Only test.
...
num_frames = 44  # Kubric / MOVi = 24.
frame_rate = 16  # Kubric / MOVi = 12.
