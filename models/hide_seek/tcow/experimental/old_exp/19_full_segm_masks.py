'''
BVH, Sep 2022.
NOTE: This is a testbed for export_kubcon_v10.
python experimental/19_full_segm_masks.py
du -sh /tmp/
rm -rf /tmp/*
'''
...
sys.path.append(os.path.join(os.getcwd(), 'data_vis/'))
...
def do_scene(scene_idx, logger, scene_dp, scene_dn):
    ...
    my_kubric = kubric_sim.MyKubricSimulatorRenderer(
        ...
        render_cpu_threads=render_cpu_threads, mass_est_fp=None)
    ...
    for perturb_idx in range(used_num_perturbs):
        ...
        for view_idx in range(used_num_views):
            ...
            # This is only an example, because given the exported data, we can easily
            ...
            save_mp4_gif(os.path.join(scene_dp, f'{scene_dn}_p{perturb_idx}_v{view_idx}_segm'),
                         data_vis.segm_to_rgb_vis(data_stack['segmentation']), frame_rate)
