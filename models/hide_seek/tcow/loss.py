...
# Internal imports.
import models.hide_seek.tcow.eval.metrics as metrics
...
class MyLosses():
    ...
    def my_voc_cls_loss(self, output_flags_logits, target_flags):
        '''
        :param output_flags_logits (B, Q, T, 2) tensor.
        :param target_flags (B, Q, T, 2) tensor.
        '''
        (B, Q, T, _) = output_flags_logits.shape

        loss_voc_cls = self.bce_loss(output_flags_logits, target_flags)
        loss_voc_cls = loss_voc_cls.mean()

        return loss_voc_cls

    def my_occl_pct_loss(self, output_pct, target_pct):
        '''
        :param output_pct (B, Q, T) tensor.
        :param target_pct (B, Q, T) tensor.
        '''
        (B, Q, T) = output_pct.shape

        loss_occl_pct = self.l1_loss(output_pct, target_pct)
        loss_occl_pct = loss_occl_pct.mean()

        return loss_occl_pct
    ...
    def per_example(self, data_retval, model_retval, progress, metrics_only):
        ...
        '''
        if 'mask_track' in self.train_args.which_seeker:
    ...
    def per_example_mask_track(self, data_retval, model_retval, progress, metrics_only):
        # occl_cont_dag = data_retval['kubric_retval']['traject_retval_tf']['occl_cont_dag_tf']
        # (B, T, M, M, 3). NOTE: Based on non-cropped data, so could be inaccurate near edges!
        # sel_query_inds = model_retval['sel_query_inds']  # (B, Q).
        # full_occl_cont_id = model_retval['full_occl_cont_id']  # (B, Q, T, 2).
        ...
        if source_name == 'kubric':
            ...
            (B, C, T, Hf, Wf) = data_retval['pv_rgb_tf'].shape
            query_time = data_retval['query_time'][0].item()
        ...
        if self.train_args.tracker_arch != 'aot':
            output_flags = model_retval['output_flags']  # (B, Q, T, 3).
            if source_name == 'kubric':
                ...
                target_flags = model_retval['target_flags']  # (B, Q, T, 3).
        ...
        if self.train_args.track_lw > 0.0:
            # This is the only loss that works for both kubric and ytvos.
            if self.train_args.tracker_arch != 'aot':
                if source_name == 'kubric':
                    ...
                    # NOTE: We do not explicitly apply any weights for AOT losses here, as the
                    # snitch prediction should a priori be considered equally important in every
                    # frame.
                    loss_track = self.my_mask_loss(
                        output_mask[:, :, 0], target_mask[:, :, 0], snitch_weights, progress, False)

                elif source_name == 'ytvos':
                    snitch_weights = torch.ones_like(target_mask[:, 0])  # (B, T, Hf, Wf).
                    snitch_weights[target_mask[:, 0] <= -0.1] = 0.0  # Either no GT or too risky.
                    loss_track = self.my_mask_loss(
                        output_mask[:, 0], target_mask[:, 0], snitch_weights, progress, False)

            else:
                loss_track = model_retval['aot_loss'].mean()
        ...
        if self.train_args.voc_cls_lw > 0.0 and source_name == 'kubric':
            # The supervision (i.e. ground truth classes) here are strongly correlated with the
            # target mask non-zero-ness.
            loss_voc_cls = self.my_voc_cls_loss(output_flags[..., 0:2], target_flags[..., 0:2])

        if self.train_args.occl_pct_lw > 0.0 and source_name == 'kubric':
            loss_occl_pct = self.my_occl_pct_loss(output_flags[..., -1], target_flags[..., -1])
        ...
        # For ytvos:
        # [(target_mask[:, 0, t].unique().tolist(), model_retval['seeker_query_mask'][:, 0, t].mean().item(), output_mask[:, 0, t].mean().item()) for t in range(T)]
        ...
        loss_retval['voc_cls'] = loss_voc_cls
        loss_retval['occl_pct'] = loss_occl_pct
        loss_retval['metrics'] = metrics_retval
    ...
    def entire_batch(self, data_retval, model_retval, loss_retval, cur_step, total_step, epoch,
        ...
        if len(loss_retval.keys()) >= 2:  # Otherwise, assume we had metrics_only enabled.
            ...
            loss_total_seeker = loss_retval['track'] * self.train_args.track_lw + \
                ...
                loss_retval['voc_cls'] * self.train_args.voc_cls_lw + \
                loss_retval['occl_pct'] * self.train_args.occl_pct_lw

            # Convert loss terms (just not the total) to floats for logging.
            for (k, v) in loss_retval.items():
                if torch.is_tensor(v):
                    loss_retval[k] = v.item()
            if not self.logger.is_none():
                ...
                if self.train_args.voc_cls_lw > 0.0:
                    self.logger.report_scalar(
                        key_prefix + 'loss_voc_cls', loss_retval['voc_cls'], **report_kwargs)
                if self.train_args.occl_pct_lw > 0.0:
                    self.logger.report_scalar(
                        key_prefix + 'loss_occl_pct', loss_retval['occl_pct'], **report_kwargs)
        ...
        # TODX DRY: This is also in metrics.py.
        for (k, v) in loss_retval['metrics'].items():
            if 'count' in k:
                ...
                if not self.logger.is_none() and new_count > 0:
