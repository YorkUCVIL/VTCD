...
https://github.com/google-research/kubric/pull/246
"""
...
def get_num_to_sample(counts):
    ...
    # results 0 and 1 are just bookkeeping; output 2 is the actual number of
...
def track_points(
    ...
    # BVH MOD
    # all_pt_3d = tf.stack(all_pt_3d)
    all_pt_3d = tf.concat(all_pt_3d, axis=0)
    ...
    return (tf.cast(chosen_points, tf.float32),
...
def add_tracks(data,
    ...
    (query_points, target_points, occluded, all_pt_3d) = track_points(
...
def create_point_tracking_dataset(
    ...
    if debug:
      ...
      # data = ds.take(1)
      for data in ds:
        retval = add_tracks(data, train_size=train_size, vflip=vflip, random_crop=random_crop)
        break
...
def plot_tracks(rgb, points, occluded, trackgroup=None):
    ...
    # rgb = (T, H, W, 3) with float values in [0, 1].
    # points = (N, T, 2) with (x, y) float values in pixel space (e.g. -64 to 298).
    # occluded = (N, T) with boolean values.
    disp = []
    cmap = plt.cm.hsv
...
def main():
  ...
  if 0:
    ...
    # data['query_points'] is (N, 3) with (i, x, y) int and float values in pixel space.
    # First column represents unique track (not object instance) ID.
    ...
    create_point_tracking_dataset(debug=True)
