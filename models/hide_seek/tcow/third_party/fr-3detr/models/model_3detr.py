...
class BoxProcessor(object):
    """
    Class to convert 3DETR MLP head results into bounding boxes
    """
    ...
    def compute_predicted_angle(self, angle_logits, angle_residual):
        if angle_logits.shape[-1] == 1:
            ...
            # we still use the predictions so that model results are used
...
class Model3DETR(nn.Module):
    ...
    - query computation: samples a set of B coordinates from the N'' points
                and results a BxD matrix of query features.
    ...
    def get_box_predictions(self, query_xyz, point_cloud_dims, box_features):
        ...
        # mlp head results are (num_layers x batch) x noutput x nqueries, so transpose last two dims
        ...
        # reshape results to num_layers x batch x nqueries x noutput
        ...
        for l in range(num_layers):
            # box processor converts results so we can get a 3D bounding box
            (
                center_normalized,
                center_unnormalized,
        ...
        # intermediate decoder layer results are only used during training
        aux_outputs = outputs[:-1]
        outputs = outputs[-1]

        return {
            "results": outputs,  # output from last layer of decoder
