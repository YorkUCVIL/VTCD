...
def train_one_epoch(
    ...
    for batch_idx, batch_data_label in enumerate(dataset_loader):
        ...
        if curr_iter % args.log_metrics_every == 0:
            ...
            # It will gather results and ground truth across all ranks.
            ...
            # results["results"] = all_gather_dict(results["results"])
...
def evaluate(
    ...
    for batch_idx, batch_data_label in enumerate(dataset_loader):
        ...
        outputs["results"] = all_gather_dict(outputs["results"])
        batch_data_label = all_gather_dict(batch_data_label)
        ap_calculator.step_meter(outputs, batch_data_label)
        time_delta.update(time.time() - curr_time)
        if is_primary() and curr_iter % args.log_every == 0:
