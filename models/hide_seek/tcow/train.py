...
def _train_one_epoch(args, train_pipeline, networks_nodp, phase, epoch, optimizers,
    ...
    for cur_step, data_retval in enumerate(tqdm.tqdm(data_loader)):
        ...
        if args.is_debug:
            ...
            if not args.data_loop_only:
                (model_retval, loss_retval) = train_pipeline[0](
                    data_retval, cur_step, total_step, epoch, progress, True, False, 'none',
                    False)
            ...
            try:
                ...
                if not args.data_loop_only:
                    # forward pass and calculate loss. not sure what function is called here
                    (model_retval, loss_retval) = train_pipeline[0](
                        data_retval, cur_step, total_step, epoch, progress, True, False, 'none',
                        False)
        ...
        if phase == 'train' and not args.data_loop_only:
            ...
            # Determine current steps to perform as part of overall optimization strategy.
            # alternation_cycle_steps = args.seeker_steps + args.hider_steps
            # step_within_cycle = cur_step % alternation_cycle_steps
            # optimize_seeker = (step_within_cycle < args.seeker_steps)
            # optimize_hider = not(optimize_seeker)
            # loss_retval['alternation_cycle_steps'] = alternation_cycle_steps
            # loss_retval['step_within_cycle'] = step_within_cycle
            # loss_retval['optimize_seeker'] = optimize_seeker
            # loss_retval['optimize_hider'] = optimize_hider
...
def main(args, logger):
    ...
    seeker_args['which_seeker'] = args.which_seeker
    ...
    seeker_args['tracker_arch'] = args.tracker_arch
    ...
    seeker_args['track_map_stride'] = args.track_map_stride
    seeker_args['track_map_resize'] = args.track_map_resize
    seeker_args['input_format'] = args.seeker_input_format
    seeker_args['query_format'] = args.seeker_query_format
    seeker_args['query_type'] = args.seeker_query_type
    seeker_args['output_format'] = args.seeker_output_format
    seeker_args['output_token'] = args.seeker_output_token
    seeker_args['output_type'] = args.seeker_output_type
    ...
    seeker_args['aot_max_gap'] = args.aot_max_gap
    seeker_args['aot_arch'] = args.aot_arch
    seeker_args['aot_pretrain_path'] = args.aot_pretrain_path
    ...
    # NOTE: I discovered the below note to be invalid, because parallel threads do not work the same
    # way as parallel processes.
    # NOTE: We can artificially increase the degree of threads / processes to match the batch size
    # precisely, which could benefit parallel Kubric generation for larger values. For this
    # to work out nicely, we need to make sure that the number of GPUs divides the batch size. For
    # example, for 2 GPUs and batch size of 6, device_ids becomes [0, 0, 0, 1, 1, 1]. The drawback
    # is that the actual neural network modules will only ever see a batch size of 1 and that GPUs
    # might experience additional concurrency / scheduling overhead.
    if args.device == 'cuda':
        train_pipeline = torch.nn.DataParallel(train_pipeline)
    
    # https://discuss.pytorch.org/t/dataparallel-freezes/36208
    # train_pipeline = train_pipeline.to(device)  # DEBUG: new
