...
class PluginVideoDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, src_path, logger, phase, num_clip_frames=20, frame_height=240,
                 ...
                 query_time=0.2, augs_version=3, annots_must_exist=False, prefetch=False,
                 center_crop=False, early_resize_height=480):
        '''
        ...
        self.augs_pipeline = augs.MyAugmentationPipeline(
            ...
            self.frame_width, self.prefer_frame_stride, False, False, False, self.augs_version,
    ...
    def __getitem__(self, index):
        ...
        self.logger.debug(
            f'(PluginVideoDataset) __getitem__: {time.time() - temp_st:.3f}s')  # DEBUG

        # Useful for debugging:
        # [np.unique(x).tolist() for x in pv_target_tf.transpose(0, 1)]
        # [np.unique(x).tolist() for x in pv_target_tf[0]]
    ...
    def _get_raw_frames(self):
        ...
        if os.path.isdir(self.src_path):
            ...
            raw_frames = frames  # NEW: list, not numpy array!

        else:
            self.logger.warning(
                f'(PluginVideoDataset) Loading {self.src_path} as a single video file...')
            # (frames, waveform, video_pts, audio_pts, metadata) = \
            #     data_utils.read_video_audio_clip(self.src_fp, None, -1, 0, False)
            # raw_frames = rearrange(frames, 'T C H W -> T H W C').numpy()
            # NOTE: Unfortunately, webm files do not seem to be supported by imageio.mimread().
            # if self.src_fp.lower().endswith('.webm'):
            #     raw_frames = imageio.mimread(self.src_fp, codec='libvpx', memtest='2GB')
            # else:
            raw_frames = imageio.mimread(self.src_fp, memtest='2GB')
            # NEW: Keep list, not numpy array!
            # raw_frames = np.array(raw_frames, dtype=np.float32) / 255.0
