...
from models.VideoMAE.modeling_finetune import Block, _cfg, PatchEmbed, get_sinusoid_encoding_table
    ...
    def forward_features(self, x, mask):
        ...
        if self.use_checkpoint:
            ...
            for block_num, blk in enumerate(self.blocks):
                x_vis, features = blk(x_vis, self.cluster_subject)

                if self.cluster_layer is not None and block_num in self.cluster_layer:
                    clust_features.append(features)

        x_vis = self.norm(x_vis)
        return x_vis, clust_features

    def forward(self, x, mask):
        x, clust_features = self.forward_features(x, mask)
        x = self.head(x)
...
class PretrainVisionTransformer(nn.Module):
    ...
    def forward(self, x, mask, return_features=True):
        B, _, T, _, _ = x.shape
        x_vis, clust_features = self.encoder(x, mask) # [B, N_vis, C_e]
        if return_features:
            if self.encoder.cluster_subject == 'attn':
                if self.encoder.attn_head == [-1]:
                ...
                clust_features = [rearrange(feat, 'B (H1 W1 T1) (H2 W2 T2) -> B T1 H1 W1 (T2 H2 W2)', B=B, T1=8, H1=14, W1=14, T2=8, H2=14, W2=14).cpu() for feat in clust_features]
            else:
                if self.encoder.attn_head == [-1]:
