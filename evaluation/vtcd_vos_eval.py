...
def compute_davis17_vos_score(vcd, first_frame_query=False, per_video_head=True,
    ...
    if post_sam:
        ...
        if 'timesformer' in vcd.args.model:
            ...
            num_frames = 16
        # reloading dataset
        vcd.dataset = vcd.load_davis17_videos(sampling_rate, num_frames)

    save_path = os.path.join(vcd.args.save_dir, 'davis17_vos_results.json')

    if os.path.exists(save_path) and not first_frame_query and not train_head_search:
        ...
        # print best heads
        for head in best_heads_norepeat:
            print(head)
        # return results
    ...
    if train_head_search:
        ...
        for idx in results.keys():
            best_heads.append(results[idx][0][0].split('concept')[0][:-1])
    ...
    if per_video_head:
        ...
        for video_idx in tqdm(range(len(vcd.labels))):
            # grab labels
            labels = vcd.labels[video_idx]
            # iterate through each object in the video
            for object_idx in range(labels.shape[1]):
                # get object labels
                single_object_label = labels[:, object_idx]
                global_object_idx += 1
                results[global_object_idx] = {}
                predictions[global_object_idx] = {}
                ...
                if first_frame_query:
                                ...
                                for concept in vcd.dic[layer][head]['concepts']:
                                    ...
                                    if isinstance(concept_masks, list):
                                        if not vcd.args.process_full_video:
                                            concept_masks = torch.stack(
                                                [torch.tensor(np.load(mask)) for mask in concept_masks],
                                                dim=0)
                                        else:
                                    ...
                                    if video_ids.sum() == 0:
                                        ...
                                        if not vcd.args.process_full_video:
                                            ...
                                            concept_masks = torch.stack(
                                                [concept_masks[i] for i in range(len(concept_masks)) if video_ids[i]],
                                                dim=0)

                                        # get single tubelet mask
                                        single_tubelet_mask = concept_masks.sum(0) > 0


                                        # compute iou for first frames
                                        iou = compute_iou(single_tubelet_mask[0], single_object_label[0])
                    ...
                    if video_idx not in video_ids:
                        ...
                        concept_masks = vcd.dic[best_dict['layer']][best_dict['head']][best_dict['concept']]['video_mask']
                        ...
                        if post_sam:
                            # postprocess masks
                            single_tubelet_mask = postprocess(concept_masks, vcd.dataset[video_idx], predictor,
                                                        num_points=num_points, mode=mode, sample_factor=sample_factor)
                        else:
                            # get single tubelet mask
                            single_tubelet_mask = concept_masks.sum(0) > 0

                        # compute iou
                        iou = compute_iou(single_tubelet_mask, single_object_label)

                        # save prediction
                        non_processed_mask = concept_masks.sum(0) > 0
                        save_prediction(vcd, vcd.dataset[video_idx], single_tubelet_mask, non_processed_mask, single_object_label, video_idx, object_idx, best_dict)
                    ...
                    results[global_object_idx][key] = iou
                    predictions[global_object_idx][key] = single_tubelet_mask
                else:
                    for layer in reversed(vcd.dic.keys()):
                        for head in vcd.args.attn_head:
                            for concept in vcd.dic[layer][head]['concepts']:
                                ...
                                if video_idx not in video_ids:
                                    ...
                                    # compute IoU between the concept_masks for video_idx and the object mask object_idx for the current video
                                    single_tubelet_mask = concept_masks.sum(0) > 0
                                    single_object_label = labels[:, object_idx]
                                    iou = compute_iou(single_tubelet_mask, single_object_label)
                                    # if iou > max_iou:
                ...
                curr_video_results = sorted(results[global_object_idx].items(), key=lambda x: x[1], reverse=True)
                print('Object: {}, best head: {}, iou: {}'.format(global_object_idx, curr_video_results[0][0], curr_video_results[0][1]))

        # sort results by iou
        for key in results.keys():
            results[key] = sorted(results[key].items(), key=lambda x: x[1], reverse=True)

        # calculate average iou over all videos for the best head
        best_ious = []
        for key in results.keys():
            best_ious.append(results[key][0][1])
